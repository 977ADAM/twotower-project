...
Архитектурная идея текущей версии: сервисные операции вынесены из `core.py` в отдельные модули и общаются с моделью через минимальные интерфейсы. Это позволяет тестировать `fit`, `predict`, `evaluate`, `save/load` без поднятия полной модели.
